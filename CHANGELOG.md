...
- Architecture docs filled for v0.1: the root `ARCHITECTURE.md` big-picture diagram upgraded from the Phase-0 stub to a Phase-1 C4 *Container* view, and `docs/architecture/` populated with four deep-dive notes (`data-model`, `posting-flow`, `outbox-flow`, `concurrency-model`) — each with an embedded Mermaid diagram and cross-links to the relevant ADRs and code paths. README "Further reading" links them.
- Production deploy config for Render (LDG-60): an `application-prod.yml` (`prod` profile) that binds Render's injected `PORT`, disables Docker Compose support, and uses a small Hikari pool for the 512 MB free instance; the datasource is supplied via `SPRING_DATASOURCE_*` env vars (never committed). The `Dockerfile` healthcheck now honours `$PORT`. README gains a Deployment section.
